...
uint16_t LCD_RD_DATA(void) {
  ...
  // 读取数据(读寄存器时,并不需要读2次)
  LCD_RD_CLR;
  if (lcddev.id == 0X8989) HAL_Delay(1);  // delay_us(2);//FOR 8989,延时2us
  t = DATAIN;
  LCD_RD_SET;
  LCD_CS_SET;
...
// 开始写GRAM
inline void LCD_WriteRAM_Prepare(void) { LCD_WR_REG(lcddev.wramcmd); }
// LCD写GRAM
// RGB_Code:颜色值
...
// 当mdk -O1时间优化时需要设置
// 延时i
inline void opt_delay(uint8_t i) { while (i--); }
// 读取个某点的颜色值
// x,y:坐标
// 返回值:此点的颜色
...
// Draw a point with color(quicker) for ILI9341 TFT-LCD
// x,y: coordinate
